...
var checkHtmlFile = function(htmlfile, checksfile) {
	return checkHtmlDocument(fs.readFileSync(htmlfile), checksfile);
};

function checkUrl(url, checksfile) {
	rest.get(url).on('complete', function(checksfile) {
	    return function(result, response) {
	        if (result instanceof Error) {
	            console.error('Error: %s', response.message);
	        } else {
				//console.error('Got html: \n' + result);
				checkJson = checkHtmlDocument(result, checksfile);
...
function checkHtmlDocument(htmlDoc, checksfile) {
